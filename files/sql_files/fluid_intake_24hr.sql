-- create fluid_intake temporary view

drop materialized view if exists fluid_intake_24hr; 
create materialized view fluid_intake_24hr as
...
--      , DATETIME_DIFF(mv.starttime, co.intime, SECOND)/60.0/60.0 as hr
      ...
      and mv.starttime-co.intime<interval '1' day  and mv.starttime>co.intime
      and
 ...
 --     , DATETIME_DIFF(cv.charttime, co.intime, SECOND)/60.0/60.0 as hr
      ...
      and cv.amount < 2000
      and cv.amountuom = 'ml'
      and cv.charttime-co.intime < interval '1' day and cv.charttime>co.intime
    )
    select
        icustay_id
      , hr
