...
#   mutate(aki_7day_new = ifelse(cre_max_7d >= admcreat + 0.3 |
#                                  cre_max_7d >= admcreat * 1.5, 1, 0))

# Inlude RRT for AKI
df <-
  df %>% mutate(crrt_start_day = as.numeric(difftime(crrt_starttime, intime, units = "days")))
df$aki_7day_new[df$crrt_start_day < 7] <- 1

# AKI Stage
df$aki_stage <- NA
df$aki_stage[df$aki_7day_new == 1] <- 1
df$aki_stage[df$cre_max_7d >= df$admcreat + 0.3 |
               df$cre_max_7d >= df$admcreat * 1.5] <- 1
