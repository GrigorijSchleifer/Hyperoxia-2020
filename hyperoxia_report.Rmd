...
## **Figure 1** Proportions in AKI based on ICU mortality
...
## **Figure 2r** AKI proportions at increasing _time weighted_ oxygen levels
...
cohort_pao2 %>%
    ...
    count(many_oxy_level, aki_7day_new) %>%
...
## *Figure 3* Proportions of AKI in the first two days grouped by ICU mortality

```{r echo=FALSE, warning=FALSE}
cohort_pao2 %>% 
    select(icu_mortality, aki_48hr) %>% 
    count(icu_mortality, aki_48hr) %>%
    group_by(icu_mortality) %>% 
    mutate(prop = n/sum(n)) %>% 
    ggplot(aes(
        fill = as.factor(aki_48hr),
        y = prop,
        x = as.factor(icu_mortality)
    )) +
    geom_bar(stat = "identity") +
    ...
    ylim(c(0, 1)) +
    labs(
        title = "Proportions in AKI in the first two days",
        subtitle = "Based on aki_48hr, grouped by icu_mortality",
        y = "proportion",
        x = "Died in the ICU",
        tag = "Figure 3",
        caption = "MIMIC data"
...
Time weighted mean PaO2 in differnent admission categories

```{r}
cohort_pao2 %>%
    group_by(admission_type) %>%
    summarise(time_weighted_mean_PaO2 = mean(time_weighted_24hr_pao2)) %>%
    mutate(admission_type = fct_reorder(admission_type, time_weighted_mean_PaO2)) %>%
    ggplot(aes(admission_type, time_weighted_mean_PaO2)) +
    geom_col() +
    coord_flip()
```
